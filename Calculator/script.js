...
function openPopup() {
  const historyString = localStorage.getItem("history"); // Get history from localStorage
  const historyArray = JSON.parse(historyString) || []; // Parse to array, or empty array if null

  // Create a simple formatted display of history
  let historyContent = "";
  historyArray.forEach((item, index) => {
    historyContent += `${index + 1}. ${item.calculation} = ${item.ans}\n`;
  });

  // Display in popup or console
  historyfun.innerText = historyContent.trim(); // Set the formatted history in the popup
}
